...
function process(req, res, pageID) {
    ...
    switch (pageID) {
        ...
        case "search":
            ...
            requestPromise({ uri: apiURL+"search", qs: { q: data.query, p: data.page }, headers: { 'Cookie': 'client_id=' + data.session.client_id + ';', 'Content-Type': 'application/json' , 'X-Forwarded-Port': req.get('X-Forwarded-Port'), 'Host': req.hostname}, json: true })
                .then((resData) => {
